    ...
    card.innerHTML = `
      ...
      <label>
        ...
        <select data-crypto-id="${escapeHTML(contract.id)}" data-crypto-field="side">
          <option value="long"${contract.side === "long" ? " selected" : ""}>做多 Long</option>
          <option value="short"${contract.side === "short" ? " selected" : ""}>做空 Short</option>
        </select>
      </label>
      <label>
        合約倍數
      ...
      <div class="crypto-position-details">
        <span>部位大小 <strong>${formatUsdt(position.notionalUsdt)}</strong></span>
        <span>BTC 數量 <strong>${formatNumber(position.quantity, 6)}</strong></span>
        <span>未實現損益 <strong>${signedUsdt(position.pnlUsdt)}</strong></span>
        <span>目前權益 <strong>${formatUsdt(position.equityUsdt)}</strong></span>
      </div>
...
function drawCategoryPieChart() {
  ...
  const width = rect.width || 320;
  const compactPie = width < 720;
  const height = compactPie ? 430 : 560;
  const totals = getPortfolioTotals();
  const slices = [
    { label: "Cash", value: totals.cash, color: assetChartColors.cashFill },
    { label: "股票", value: totals.stockTotal, color: assetChartColors.stockFill },
    { label: "基金", value: totals.fundTotal, color: assetChartColors.fundFill },
    { label: "加密合約", value: totals.cryptoTotal, color: assetChartColors.cryptoFill },
  ].filter((slice) => slice.value > 0);
  const total = slices.reduce((sum, slice) => sum + slice.value, 0);
  ...
  drawCategoryPieLegend(context, slices, width);
  const radius = compactPie ? Math.min((width - 188) / 2, 76) : Math.min((width - 360) / 2, 190);
  const centerX = width / 2;
  ...
  if (compactPie) {
    drawMobileOutsidePieLabels(context, outsideLabels, centerX, centerY, radius, height, width);
  } else {
    drawOutsidePieLabels(context, outsideLabels, centerX, radius, height);
  }
}

function drawCategoryPieLegend(context, slices, width) {
  const itemWidth = width / 4;
  context.font = width < 720 ? "10px sans-serif" : "12px sans-serif";
  context.textAlign = "left";
  context.textBaseline = "middle";
  slices.forEach((slice) => {
    const order = ["Cash", "股票", "基金", "加密合約"].indexOf(slice.label);
    const x = 12 + Math.max(0, order) * itemWidth;
    context.fillStyle = slice.color;
    context.fillRect(x, 18, 10, 10);
    context.fillStyle = "#6c756f";
    context.fillText(slice.label, x + 14, 23);
  });
}
...
function calculateCryptoPosition(contract) {
  ...
  const quantity = entryPrice ? notionalUsdt / entryPrice : 0;
    ...
    ? value.cryptoContracts.map((contract) => {
        ...
        if (Number.isFinite(Number(contract.marginUsdt))) {
          return {
            ...
            leverage,
            marginUsdt: toNumber(contract.marginUsdt),
            entryPrice: toNumber(contract.entryPrice),
