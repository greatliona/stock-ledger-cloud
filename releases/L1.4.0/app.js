...
const els = {
  ...
  usName: document.querySelector("#usNameInput"),
  ...
  usFxRate: document.querySelector("#usFxRateInput"),
...
els.usHoldingsBody.addEventListener("change", (event) => {
  ...
  const fxInput = event.target.closest("[data-us-fx-id]");
  if (!priceInput && !fxInput) return;
  const holding = findUsHolding(priceInput?.dataset.usPriceId || fxInput?.dataset.usFxId);
  if (!holding) return;
  if (priceInput) holding.currentPrice = toOptionalNumber(priceInput.value);
  if (fxInput) holding.fxRate = toNumber(fxInput.value);
...
els.usHoldingsBody.addEventListener("click", (event) => {
  ...
  if (saveBtn) {
    ...
    if (!holding || !editRow) return;
    holding.name = editRow.querySelector("[data-edit-us-field='name']").value.trim();
    ...
    holding.fxRate = toNumber(editRow.querySelector("[data-edit-us-field='fxRate']").value);
    editingUsHoldingId = "";
    saveAndRender("已更新美股資料。");
    return;
  }
  ...
  if (!holding || !confirm(`確定刪除 ${holding.name || holding.symbol || "這筆美股"}？`)) return;
...
els.settleBtn.addEventListener("click", () => {
  state.cash = toNumber(els.cash.value);
  if (!state.holdings.length && !state.usHoldings.length && !state.funds.length && !state.cryptoContracts.length && !state.cash) {
  ...
  for (const input of els.usHoldingsBody.querySelectorAll("[data-us-price-id], [data-us-fx-id]")) {
    const holding = findUsHolding(input.dataset.usPriceId || input.dataset.usFxId);
    if (!holding) continue;
    if (input.dataset.usPriceId) holding.currentPrice = toOptionalNumber(input.value);
    if (input.dataset.usFxId) holding.fxRate = toNumber(input.value);
  }
  ...
  for (const holding of state.usHoldings) {
    const price = holding.currentPrice ?? holding.avgCost;
    const fxRate = holding.fxRate || 0;
    usPrices[holding.id] = { price, fxRate };
    holding.currentPrice = price;
    usStockTotal += holding.shares * price * fxRate;
  ...
  const record = {
    ...
    cryptoCost,
    cost,
    pnl: stockTotal + usStockTotal + fundTotal + cryptoTotal - cost,
    prices,
    usPrices,
    fundValues,
    cryptoValues,
...
els.historyList.addEventListener("click", (event) => {
  ...
  if (action === "save") {
    ...
    record.pnl = stockTotal + usStockTotal + fundTotal + cryptoTotal - record.cost;
    record.note = note;
    state.history.sort((a, b) => a.date.localeCompare(b.date));
    editingHistoryDate = "";
...
function renderUsHoldings() {
  ...
  if (!state.usHoldings.length) {
    const row = document.createElement("tr");
    row.innerHTML = '<td colspan="9" class="empty-cell">尚未新增美股</td>';
  ...
  for (const holding of [...state.usHoldings].sort(compareHoldingsByDisplayName)) {
    ...
    const row = document.createElement("tr");
    row.innerHTML = `
      <td><strong>${escapeHTML(holding.name || holding.symbol)}</strong><span class="stock-name">${escapeHTML(holding.symbol)}</span></td>
      <td class="shares-cell">${formatNumber(holding.shares, 3)}</td>
      ...
      <td class="fx-cell"><input data-us-fx-id="${escapeHTML(holding.id)}" type="number" min="0" step="0.001" value="${holding.fxRate ?? ""}" aria-label="${escapeHTML(holding.symbol)} 台幣匯率" /></td>
      <td class="amount-cell">${money(costValue)}</td>
      <td class="amount-cell">${money(marketValue)}</td>
    ...
    if (editingUsHoldingId === holding.id) {
      ...
      editRow.innerHTML = `<td colspan="9"><div class="edit-holding-form us-edit-holding-form">
        <label>名稱<input data-edit-us-field="name" autocomplete="off" value="${escapeHTML(holding.name || "")}" /></label>
        ...
        <label>台幣匯率<input data-edit-us-field="fxRate" type="number" min="0" step="0.001" value="${holding.fxRate}" /></label>
...
function drawUsStockPieChart() {
  ...
  const slices = state.usHoldings
    .map((holding) => ({
      label: holding.name || holding.symbol,
      group: holding.name || holding.symbol,
      value: holding.shares * (holding.currentPrice ?? holding.avgCost) * (holding.fxRate || 0),
    }))
    .filter((slice) => slice.value > 0)
    .sort((a, b) => b.value - a.value || chineseNameSorter.compare(a.label, b.label));
  const groupColorFamilies = assignPieColorFamilies(slices);
...
function getPortfolioTotals() {
  ...
  const usStockCost = state.usHoldings.reduce(
    (sum, item) => sum + item.shares * item.avgCost * (item.fxRate || 0),
    0,
  );
  const usStockTotal = state.usHoldings.reduce(
    (sum, item) => sum + item.shares * (item.currentPrice ?? item.avgCost) * (item.fxRate || 0),
    0,
  );
  ...
  const usStockPnl = usStockTotal - usStockCost;
  ...
  return { cost, value, cash, stockTotal, usStockTotal, fundTotal, cryptoTotal, stockCost, usStockCost, fundCost, cryptoCost, stockPnl, usStockPnl, fundPnl, cryptoPnl, stockPnlPct, usStockPnlPct, fundPnlPct, cryptoPnlPct, pnl, pnlPct };
}
...
function backfillHistoryCosts() {
  ...
  for (const item of state.history) {
    ...
    const nextPnl = toNumber(item.stockTotal) + toNumber(item.usStockTotal) + toNumber(item.fundTotal) + toNumber(item.cryptoTotal) - nextCost;
...
function loadState() {
  const fallback = { holdings: [], usHoldings: [], funds: [], cryptoContracts: [], history: [], currentFundTotal: 0, cash: 0 };
  try {
...
function migrateState(value) {
  ...
  const usHoldings = Array.isArray(value.usHoldings)
    ? value.usHoldings.map((holding) => ({
        ...
        avgCost: toNumber(holding.avgCost),
        currentPrice: toOptionalNumber(holding.currentPrice),
        fxRate: toNumber(holding.fxRate),
        lastUpdated: String(holding.lastUpdated || ""),
      }))
    : [];
