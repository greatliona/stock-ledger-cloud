...
const els = {
  ...
  cryptoCostInput: document.querySelector("#cryptoCostInput"),
  cryptoAmount: document.querySelector("#cryptoAmountInput"),
...
els.cryptoForm.addEventListener("submit", (event) => {
  ...
  state.cryptoContracts.push({
    id: crypto.randomUUID(),
    name: els.cryptoName.value.trim(),
    leverage: Math.max(1, toNumber(els.cryptoLeverage.value)),
    cost: toNumber(els.cryptoCostInput.value),
    currentValue: toNumber(els.cryptoAmount.value),
  });
...
els.cryptoList.addEventListener("change", (event) => {
  ...
  contract[field] = field === "leverage" ? Math.max(1, toNumber(input.value)) : toNumber(input.value);
...
els.settleBtn.addEventListener("click", () => {
  ...
  for (const contract of state.cryptoContracts) {
    const value = contract.currentValue ?? 0;
    cryptoValues[contract.id] = value;
    cryptoTotal += value;
    cryptoCost += contract.cost || 0;
  }
...
function renderCryptoContracts() {
  ...
  for (const contract of state.cryptoContracts) {
    const pnl = contract.currentValue - contract.cost;
    const pnlPct = contract.cost ? (pnl / contract.cost) * 100 : 0;
    const card = document.createElement("article");
    card.className = "crypto-card";
    card.innerHTML = `
      <div class="crypto-card-head">
        <strong>${escapeHTML(contract.name)}</strong>
        <label class="crypto-leverage">
          合約倍數
          <input data-crypto-id="${escapeHTML(contract.id)}" data-crypto-field="leverage" type="number" min="1" step="1" value="${contract.leverage}" aria-label="${escapeHTML(contract.name)} 合約倍數" />
        </label>
      </div>
      <label>
        開倉金額
        <input data-crypto-id="${escapeHTML(contract.id)}" data-crypto-field="cost" type="number" min="0" step="0.01" value="${contract.cost}" />
      </label>
      <label>
        目前金額
        <input data-crypto-id="${escapeHTML(contract.id)}" data-crypto-field="currentValue" type="number" min="0" step="0.01" value="${contract.currentValue}" />
      </label>
      <div class="crypto-card-bottom ${pnl >= 0 ? "gain" : "loss"}">
        <div class="fund-pnl-line">
          <span>${signedMoney(pnl)}</span>
          <span class="stock-name">${signedPercent(pnlPct)}</span>
        </div>
...
function drawAssetChart(canvas, mode = "combined") {
  ...
  const points = history.map((item, index) => {
    ...
    const cryptoY = padding.top + chartHeight - ((cryptoTotal - min) / range) * chartHeight;
    const fundY = padding.top + chartHeight - ((cryptoTotal + fundTotal - min) / range) * chartHeight;
    const stockY = padding.top + chartHeight - ((cryptoTotal + fundTotal + stockTotal - min) / range) * chartHeight;
  ...
  if (mode === "combined") {
    if (points.length === 1) {
      ...
      fillStackedArea(context, points, padding.top + chartHeight, "cryptoY", assetChartColors.cryptoFill);
      fillStackedBand(context, points, "fundY", "cryptoY", assetChartColors.fundFill);
...
function drawSingleStackedColumn(context, point, padding, chartWidth, chartHeight) {
  ...
  context.fillRect(x, point.cryptoY, columnWidth, baselineY - point.cryptoY);
  context.fillStyle = assetChartColors.fundFill;
  context.fillRect(x, point.fundY, columnWidth, point.cryptoY - point.fundY);
  ...
  context.strokeRect(x, point.cryptoY, columnWidth, baselineY - point.cryptoY);
  context.strokeStyle = assetChartColors.fundLine;
  context.strokeRect(x, point.fundY, columnWidth, point.cryptoY - point.fundY);
...
function getPortfolioTotals() {
  ...
  const cryptoTotal = state.cryptoContracts.reduce((sum, item) => sum + (item.currentValue || 0), 0);
  const cryptoCost = state.cryptoContracts.reduce((sum, item) => sum + (item.cost || 0), 0);
  const cash = toNumber(state.cash);
  const cost = stockCost + fundCost + cryptoCost;
...
function migrateState(value) {
  ...
  const cryptoContracts = Array.isArray(value.cryptoContracts)
    ? value.cryptoContracts.map((contract) => ({
        ...
        name: String(contract.name || "未命名合約"),
        leverage: Math.max(1, toNumber(contract.leverage)),
        cost: toNumber(contract.cost),
        currentValue: toNumber(contract.currentValue),
      }))
    : [];
