  ...
  for (const [index, item] of historyItems.entries()) {
    ...
    if (editingHistoryDate === item.date) {
      ...
      node.innerHTML = `
        ...
        <em>台灣股市 ${money(item.stockTotal || 0)} ${dailyChange(item.stockTotal, previousItem?.stockTotal)}</em>
        <em>基金 ${money(item.fundTotal || 0)} ${dailyChange(item.fundTotal, previousItem?.fundTotal)}</em>
        <em>加密 ${money(item.cryptoTotal || 0)} ${dailyChange(item.cryptoTotal, previousItem?.cryptoTotal)}</em>
        <em>美國股市 ${money(item.usStockTotal || 0)} ${dailyChange(item.usStockTotal, previousItem?.usStockTotal)}</em>
        <em>損益 ${signedMoney(item.pnl || 0)} ${dailyChange(item.pnl, previousItem?.pnl)}</em>
