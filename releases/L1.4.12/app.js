  ...
  for (const holding of sortedHoldings) {
    ...
    row.innerHTML = `
      ...
      <td class="pnl-cell ${pnl >= 0 ? "gain" : "loss"}">
        <div class="stock-pnl-layout">
          <div class="stock-pnl-value">${signedMoney(pnl)}<span class="stock-name">${formatNumber(pnlPct)}%</span></div>
...
function renderUsHoldings() {
  ...
  if (!state.usHoldings.length) {
    const row = document.createElement("tr");
    row.innerHTML = '<td colspan="7" class="empty-cell">尚未新增美股</td>';
  ...
  for (const holding of [...state.usHoldings].sort(compareHoldingsByDisplayName)) {
    ...
    row.innerHTML = `
      ...
      <td class="amount-cell">${money(costValue)}</td>
      <td class="amount-cell">${money(marketValue)}</td>
      <td class="pnl-cell ${pnl >= 0 ? "gain" : "loss"}"><div class="stock-pnl-layout"><div class="stock-pnl-value">${signedMoney(pnl)}<span class="stock-name">${signedPercent(pnlPct)}</span></div><div class="row-actions"><button class="mini-btn" data-edit-us-id="${escapeHTML(holding.id)}" type="button" aria-label="修改 ${escapeHTML(holding.symbol)}">${editIcon()}</button><button class="mini-btn" data-delete-us-id="${escapeHTML(holding.id)}" type="button" aria-label="刪除 ${escapeHTML(holding.symbol)}">x</button></div></div></td>
    `;
    els.usHoldingsBody.append(row);
    ...
    if (editingUsHoldingId === holding.id) {
      ...
      editRow.innerHTML = `<td colspan="7"><div class="edit-holding-form us-edit-holding-form">
  ...
  for (const [index, item] of historyItems.entries()) {
    ...
    if (editingHistoryDate === item.date) {
      ...
      node.innerHTML = `
        ...
        <em>台灣股市 ${money(item.stockTotal || 0)}</em>
        <em>基金 ${money(item.fundTotal || 0)}</em>
        <em>加密 ${money(item.cryptoTotal || 0)}</em>
        <em>美國股市 ${money(item.usStockTotal || 0)}</em>
