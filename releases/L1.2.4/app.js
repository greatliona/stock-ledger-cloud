...
function renderHistory() {
  ...
  for (const item of historyItems) {
    ...
    if (monthLists.has(month)) continue;

    const monthItems = historyItems.filter((historyItem) => String(historyItem.date || "").startsWith(month));
    const monthPanel = document.createElement("details");
    monthPanel.className = "history-month";
    monthPanel.open = Boolean(editingHistoryDate && editingHistoryDate.startsWith(month));
    monthPanel.innerHTML = `
      <summary>${escapeHTML(month)}<span>${monthItems.length} 筆</span></summary>
      <div class="history-month-list"></div>
    `;
    els.historyList.append(monthPanel);
    monthLists.set(month, monthPanel.querySelector(".history-month-list"));
  }
