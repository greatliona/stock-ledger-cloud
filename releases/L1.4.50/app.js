...
function renderHistory() {
  ...
  if (window.matchMedia("(max-width: 640px)").matches) {
    window.requestAnimationFrame(() => {
      monthTabs.scrollLeft = monthTabs.scrollWidth - monthTabs.clientWidth;
    });
  }
...
function drawPieChart() {
  ...
  const chartHeight = compactPie ? 560 : 800;
...
function drawPieLeaderLine(context, label, centerX, centerY, markerX, markerY, compact) {
  ...
  const needsBend = directX * outwardX + directY * outwardY < 0;
...
function drawSeparatedInsidePieLabels(context, labels, centerX, centerY, radius, font, lineHeight) {
  ...
  for (const label of labels) {
    ...
    const radiusFactors = label.percent < 10
      ? [0.62, 0.74, 0.5, 0.82, 0.4]
      : [0.58, 0.7, 0.46];
    const tangentOffsets = [0, -10, 10, -18, 18];
    let best = null;

    for (const factor of radiusFactors) {
      for (const tangentOffset of tangentOffsets) {
        const baseX = centerX + Math.cos(label.mid) * radius * factor;
        const baseY = centerY + Math.sin(label.mid) * radius * factor;
        const x = baseX - Math.sin(label.mid) * tangentOffset;
        const y = baseY + Math.cos(label.mid) * tangentOffset;
        const rect = {
...
function positionPieLabels(sideLabels, centerY, top, bottom, minGap, clearance) {
  const sorted = [...sideLabels].sort((a, b) => a.anchorY - b.anchorY);
  const topLabels = sorted.filter((label) => label.anchorY < centerY);
  const bottomLabels = sorted.filter((label) => label.anchorY >= centerY);

  if (topLabels.length) {
    const lastAnchorY = topLabels[topLabels.length - 1].anchorY;
    const endY = Math.min(centerY - clearance, lastAnchorY - clearance);
    const gap = Math.min(minGap, (endY - top) / Math.max(1, topLabels.length - 1));
    topLabels.forEach((label, index) => {
      label.markerY = top + Math.max(0, gap) * index;
    });
  }

  if (bottomLabels.length) {
    const firstAnchorY = bottomLabels[0].anchorY;
    const startY = Math.max(centerY + clearance, firstAnchorY + clearance);
    const gap = Math.min(minGap, (bottom - startY) / Math.max(1, bottomLabels.length - 1));
    bottomLabels.forEach((label, index) => {
      label.markerY = startY + Math.max(0, gap) * index;
    });
  }
...
function positionDesktopPieLabels(sideLabels, centerY, top, bottom, minGap) {
  const sorted = [...sideLabels].sort((a, b) => a.anchorY - b.anchorY);
  const topLabels = sorted.filter((label) => label.anchorY < centerY);
  const bottomLabels = sorted.filter((label) => label.anchorY >= centerY);

  if (topLabels.length) {
    const placeTopLabels = (gap) => {
      const positions = new Array(topLabels.length);
      let nextY = topLabels[topLabels.length - 1].anchorY + gap;
      for (let index = topLabels.length - 1; index >= 0; index -= 1) {
        positions[index] = Math.min(topLabels[index].anchorY, nextY - gap);
        nextY = positions[index];
      }
      return positions;
    };
    let lowGap = 0;
    ...
    for (let step = 0; step < 12; step += 1) {
      const gap = (lowGap + highGap) / 2;
      if (placeTopLabels(gap)[0] >= top) lowGap = gap;
      else highGap = gap;
    }
    placeTopLabels(lowGap).forEach((markerY, index) => {
      topLabels[index].markerY = markerY;
    });
  }

  if (bottomLabels.length) {
    const placeBottomLabels = (gap) => {
      const positions = new Array(bottomLabels.length);
      let previousY = bottomLabels[0].anchorY - gap;
      for (let index = 0; index < bottomLabels.length; index += 1) {
        positions[index] = Math.max(bottomLabels[index].anchorY, previousY + gap);
        previousY = positions[index];
      }
      return positions;
    };
    let lowGap = 0;
    ...
    for (let step = 0; step < 12; step += 1) {
      const gap = (lowGap + highGap) / 2;
      const positions = placeBottomLabels(gap);
      if (positions[positions.length - 1] <= bottom) lowGap = gap;
      else highGap = gap;
    }
    placeBottomLabels(lowGap).forEach((markerY, index) => {
      bottomLabels[index].markerY = markerY;
    });
  }
...
function drawOutsidePieLabels(context, labels, centerX, radius, height) {
  ...
  for (const side of ["left", "right"]) {
    const sideLabels = positionDesktopPieLabels(
      labels.filter((label) => label.side === side),
      centerY,
      top,
      bottom,
