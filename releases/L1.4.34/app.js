...
function drawPieChart() {
  ...
  const summaryHeight = getPieGroupSummaryHeight(summaryRows, width, compactPie, 2);
  ...
  drawPieGroupSummary(context, summaryRows, width, chartHeight, summaryHeight, compactPie, "中文股名小計", false, 2);
}
...
function drawPieGroupSummary(context, rows, width, chartHeight, summaryHeight, compactPie, title = "中文股名小計", flowItems = false, mobileColumns = 1) {
  ...
  if (compactPie) {
    drawMobilePieGroupSummary(context, rows, x, y, panelWidth, title, flowItems, mobileColumns);
    return;
  }
...
function drawMobilePieGroupSummary(context, rows, x, y, width, title = "中文股名小計", showFullLabels = false, columns = 1) {
  const padding = 11;
  const rowGap = 20;
  let rowY = y + 18;
  ...
  rows.forEach((row, index) => {
    ...
    const itemY = rowY + line * rowGap;
    const label = `${row.group}: ${formatNumber(row.percent)}% ${money(row.value)}`;
    const maxLabelWidth = columnWidth - 14;
    let fontSize = 10.5;
    ...
    context.fillStyle = row.color;
    context.fillRect(itemX, itemY - 4, 8, 8);
    context.strokeStyle = "rgba(23, 33, 28, 0.25)";
    context.strokeRect(itemX, itemY - 4, 8, 8);

    context.fillStyle = "#17211c";
    context.fillText(showFullLabels ? label : trimCanvasText(context, label, maxLabelWidth), itemX + 13, itemY);
  });
}

function getPieGroupSummaryHeight(rows, width, compactPie, columns = 1) {
  if (compactPie) return Math.max(174, 48 + Math.ceil(rows.length / columns) * 20);
