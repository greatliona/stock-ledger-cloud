...
els.historyList.addEventListener("click", (event) => {
  ...
  if (monthToggle) {
    ...
    openHistoryMonth = openHistoryMonth === month ? "" : month;
    renderHistory();
    return;
  }
  ...
  if (action === "unlock") {
    ...
    openHistoryMonth = String(date || "").slice(0, 7);
    renderHistory();
    return;
  }
  ...
  if (action === "save") {
    ...
    state.history.sort((a, b) => a.date.localeCompare(b.date));
    editingHistoryDate = "";
    openHistoryMonth = String(nextDate || "").slice(0, 7);
...
function renderHistory() {
  ...
  for (const [month] of monthGroups) {
    const isOpen = openHistoryMonth === month || Boolean(editingHistoryDate && editingHistoryDate.startsWith(month));
