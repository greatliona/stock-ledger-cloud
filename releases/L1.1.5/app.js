...
function drawAssetChart(canvas, mode = "combined") {
  ...
  const points = history.map((item, index) => {
    ...
    const fundY = padding.top + chartHeight - ((stockTotal + fundTotal - min) / range) * chartHeight;
  ...
  if (mode === "combined") {
    if (points.length === 1) {
      ...
      fillStackedBand(context, points, "fundY", "stockY", assetChartColors.fundFill);
      fillStackedBand(context, points, "totalY", "fundY", assetChartColors.cashFill);
    }
  } else {
  ...
  if (mode === "combined") {
    ...
    drawAssetLine(context, points, "totalY", assetChartColors.cashLine, 4.5);
  } else {
...
function drawSingleStackedColumn(context, point, padding, chartWidth, chartHeight) {
  ...
  context.fillRect(x, point.fundY, columnWidth, point.stockY - point.fundY);
  context.fillStyle = assetChartColors.cashFill;
  context.fillRect(x, point.totalY, columnWidth, point.fundY - point.totalY);
  ...
  context.strokeRect(x, point.fundY, columnWidth, point.stockY - point.fundY);
  context.strokeStyle = assetChartColors.cashLine;
  context.strokeRect(x, point.totalY, columnWidth, point.fundY - point.totalY);
}
...
function backfillHistoryCosts() {
  ...
  for (const item of state.history) {
    ...
    const nextTotal = toNumber(item.stockTotal) + toNumber(item.fundTotal) + toNumber(item.cash);
...
function migrateState(value) {
  ...
  const history = value.history
    .map((item) => {
      ...
      const cash = Number.isFinite(Number(item.cash)) ? toNumber(item.cash) : Math.max(toNumber(item.total) - stockTotal - fundTotal, 0);
