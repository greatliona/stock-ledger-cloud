...
function drawPieChart() {
  ...
  for (const slice of slices) {
    ...
    if (percent >= (compactPie ? 16 : 12)) {
      const labelRadius = radius * getInsidePieLabelRadiusFactor(mid, insideLabelMids, compactPie, 0.58);
...
function drawCategoryPieChart() {
  ...
  for (const slice of slices) {
    ...
    const lines = [slice.label, `${formatNumber(percent)}%`, money(slice.value)];
    if (compactPie) {
      insideLabels.push({ lines, mid, percent });
    } else if (percent >= 12) {
      const labelRadius = radius * getInsidePieLabelRadiusFactor(mid, insideLabelMids, compactPie, 0.58);
      context.fillStyle = "#17211c";
...
function drawCryptoPieChart() {
  ...
  for (const slice of slices) {
    ...
    if (compactPie || percent >= 12) {
      ...
      const labelRadius = radius * getInsidePieLabelRadiusFactor(mid, insideLabelMids, compactPie, baseFactor);
      context.fillStyle = "#17211c";
      context.font = compactPie ? "700 8px sans-serif" : "700 11.5px sans-serif";
      context.textAlign = "center";
      context.textBaseline = "middle";
...
function drawSeparatedInsidePieLabels(context, labels, centerX, centerY, radius, font, lineHeight) {
  ...
  for (const label of labels) {
    ...
    const preferredFactor = getInsidePieLabelRadiusFactor(label.mid, labelMids, true, baseFactor);
...
function getInsidePieLabelRadiusFactor(mid, labelMids, compactPie, baseFactor) {
  ...
  return nearestGap >= isolatedGap ? (compactPie ? 0.72 : 0.7) : baseFactor;
}
