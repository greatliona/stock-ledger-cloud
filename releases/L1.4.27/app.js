  ...
  for (const [index, item] of historyItems.entries()) {
    ...
    if (editingHistoryDate === item.date) {
      ...
      node.innerHTML = `
        ...
        <em class="history-stock-row"><span>台股 ${money(item.stockTotal || 0)}</span>${dailyChange(item.stockTotal, previousItem?.stockTotal)}</em>
        ...
        <em>美股 ${money(item.usStockTotal || 0)} ${dailyChange(item.usStockTotal, previousItem?.usStockTotal)}</em>
