...
function drawPieLeaderLine(context, label, centerX, centerY, radius, markerX, markerY, side, compact) {
  ...
  const radialLength = compact ? 10 : 18;
  const bendInset = compact ? 8 : 18;
  const radialX = label.anchorX + ((label.anchorX - centerX) / radius) * radialLength;
  const radialY = label.anchorY + ((label.anchorY - centerY) / radius) * radialLength;
  const bendX = markerX - direction * bendInset;

  context.strokeStyle = label.color;
  context.lineWidth = compact ? 2.6 : 3.2;
  context.beginPath();
  context.moveTo(label.anchorX, label.anchorY);
  context.lineTo(radialX, radialY);
  context.stroke();

  context.strokeStyle = "rgba(23, 33, 28, 0.62)";
  context.lineWidth = compact ? 0.9 : 1.1;
  context.lineJoin = "round";
  context.beginPath();
  context.moveTo(label.anchorX, label.anchorY);
  context.lineTo(radialX, radialY);
  context.lineTo(bendX, markerY);
  context.lineTo(markerX, markerY);
  context.stroke();

  context.fillStyle = label.color;
  context.strokeStyle = "rgba(23, 33, 28, 0.45)";
  context.lineWidth = 0.8;
  context.beginPath();
  context.arc(label.anchorX, label.anchorY, compact ? 1.8 : 2.4, 0, Math.PI * 2);
  context.fill();
  context.stroke();
}
...
function drawMobileOutsidePieLabels(context, labels, centerX, centerY, radius, height, width) {
  ...
  for (const side of ["left", "right"]) {
    ...
    const effectiveGap = Math.min(minGap, availableHeight / Math.max(1, sideLabels.length - 1));
    let previousY = top - effectiveGap;

    for (const label of sideLabels) {
      const targetY = Math.min(bottom, Math.max(top, label.targetY));
      label.y = Math.max(targetY, previousY + effectiveGap);
      previousY = label.y;
    }

    const last = sideLabels[sideLabels.length - 1];
    last.y = Math.min(last.y, bottom);

    for (let index = sideLabels.length - 2; index >= 0; index -= 1) {
      const next = sideLabels[index + 1];
      sideLabels[index].y = Math.max(top, Math.min(sideLabels[index].y, next.y - effectiveGap));
    }
    ...
    const labelInset = Math.min(width / 2 - 18, Math.max(84, maxLineWidth + 14));
...
function drawOutsidePieLabels(context, labels, centerX, radius, height) {
  ...
  for (const side of ["left", "right"]) {
    ...
    for (const label of sideLabels) {
      ...
      drawPieLeaderLine(context, label, centerX, height / 2, radius, markerX, markerY, side, false);
