  ...
  for (const [index, item] of historyItems.entries()) {
    ...
    if (editingHistoryDate === item.date) {
      ...
      const dailyChange = (currentValue, previousValue) => {
        ...
        return `<small class="history-daily-pnl history-inline-daily">${shortChange} ${percent >= 0 ? "+" : ""}${formatNumber(percent, 2)}%</small>`;
      };
      node.innerHTML = `
...
function drawAssetChart(canvas, mode = "combined") {
  ...
  if (history.length === 0) {
    updateAssetChartHoverData(canvas, mode, []);
...
function setupAssetChartHover(canvas) {
  ...
  canvas.addEventListener("mousemove", (event) => {
    ...
    const nearest = findHoveredAssetChartPoint(interaction.points, interaction.mode, pointerX, pointerY);
    if (!nearest) {
      hideAssetChartTooltip(canvas);
      return;
    }

    showAssetChartTooltip(canvas, nearest.point, nearest.y);
  });
  canvas.addEventListener("mouseleave", () => hideAssetChartTooltip(canvas));
}

function updateAssetChartHoverData(canvas, mode, points) {
  const interaction = assetChartInteractions.get(canvas);
...
function getAssetChartHoverTargets(point, mode) {
  if (mode === "combined") {
    return [
      point.fundY,
      point.cryptoTotal > 0 ? point.cryptoY : null,
      point.stockY,
      point.cash > 0 ? point.totalY : null,
    ].filter(Number.isFinite);
  }
  return [point.valueY];
}

function findHoveredAssetChartPoint(points, mode, pointerX, pointerY) {
  let nearest = null;
  for (const point of points) {
    for (const y of getAssetChartHoverTargets(point, mode)) {
      const distance = Math.hypot(pointerX - point.x, pointerY - y);
      if (!nearest || distance < nearest.distance) nearest = { point, y, distance };
    }
  }
  return nearest?.distance <= 16 ? nearest : null;
}
...
function showAssetChartTooltip(canvas, point, pointY) {
  const interaction = assetChartInteractions.get(canvas);
  const slide = canvas.closest(".asset-chart-slide");
  if (!interaction || !slide) return;
  ...
  const canvasLeft = canvasRect.left - slideRect.left;
  const canvasTop = canvasRect.top - slideRect.top;
  const openLeft = point.x > canvasRect.width * 0.64;
  const preferredLeft = canvasLeft + point.x + (openLeft ? -tooltipWidth - 14 : 14);
  const preferredTop = canvasTop + pointY - tooltipHeight / 2;
  const left = Math.min(slide.clientWidth - tooltipWidth - 8, Math.max(8, preferredLeft));
  const top = Math.min(slide.clientHeight - tooltipHeight - 8, Math.max(8, preferredTop));
...
function drawSeparatedInsidePieLabels(context, labels, centerX, centerY, radius, font, lineHeight) {
  ...
  for (const label of labels) {
    ...
    const radiusFactors = label.percent < 10
