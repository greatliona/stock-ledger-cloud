  ...
  for (const [index, item] of historyItems.entries()) {
    ...
    if (editingHistoryDate === item.date) {
      ...
      node.innerHTML = `
        ...
        <em class="history-value-row"><span>台股 ${money(item.stockTotal || 0)}</span>${dailyChange(item.stockTotal, previousItem?.stockTotal)}</em>
        <em class="history-value-row"><span>基金 ${money(item.fundTotal || 0)}</span>${dailyChange(item.fundTotal, previousItem?.fundTotal)}</em>
        <em class="history-value-row"><span>加密 ${money(item.cryptoTotal || 0)}</span>${dailyChange(item.cryptoTotal, previousItem?.cryptoTotal)}</em>
        <em class="history-value-row"><span>美股 ${money(item.usStockTotal || 0)}</span>${dailyChange(item.usStockTotal, previousItem?.usStockTotal)}</em>
