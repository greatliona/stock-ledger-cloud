...
function drawCryptoPieChart() {
  ...
  const summaryHeight = getPieGroupSummaryHeight(summaryRows, width, compactPie, summaryColumnCount);
  const height = chartHeight + summaryHeight;
  ...
  drawPieGroupSummary(context, summaryRows, width, chartHeight, summaryHeight, compactPie, "加密合約小計", summaryColumnCount);
}
...
function drawPieGroupSummary(context, rows, width, chartHeight, summaryHeight, compactPie, title = "中文股名小計", desktopColumnCount = 7) {
  ...
  const columnCount = desktopColumnCount;
...
function getPieGroupSummaryHeight(rows, width, compactPie, desktopColumnCount = 7) {
  if (compactPie) return Math.max(174, 48 + rows.length * 20);
  const lineCount = Math.max(1, Math.ceil(rows.length / desktopColumnCount));
