...
function openTradeDecision(market, holding) {
  ...
  const marketLabel = isUs ? "美股" : "台股";
  const displayName = holding.name ? `${holding.name} ${holding.symbol}` : holding.symbol;
  els.tradeDecisionStock.textContent = `${marketLabel} · ${displayName}`;
...
function renderTradeRecord(trade) {
  const marketLabel = trade.market === "us" ? "美股" : "台股";
  ...
  return `
    <span class="trade-record-field">
      <small>市場／股票</small>
      <span class="trade-record-symbol">${marketLabel} · ${escapeHTML(trade.symbol)}</span>
      ${trade.name ? `<span class="trade-record-name">${escapeHTML(trade.name)}</span>` : ""}
    </span>
...
function openDeleteTradeDialog(trade) {
  ...
  els.deleteTradeDescription.textContent = `${trade.market === "us" ? "美股" : "台股"} · ${trade.symbol} · ${trade.buyDate} → ${trade.sellDate} · ${signedMoney(trade.pnlTwd)}`;
...
function exportTradeHistoryExcel() {
  ...
  window.XLSX.writeFile(workbook, `stock-trades-${todayISO()}.xlsx`, { compression: true });
  setStatus("交易紀錄 Excel 已匯出。" );
}
