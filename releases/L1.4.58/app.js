...
function setupAssetChartHover(canvas) {
  ...
  tooltip.dataset.mode = canvas.dataset.assetChart || "combined";
  tooltip.setAttribute("aria-hidden", "true");
  slide.insertBefore(tooltip, canvas);
  ...
  canvas.addEventListener("mousemove", (event) => {
    ...
    showAssetChartTooltip(canvas, nearest.point);
  });
...
function updateAssetChartHoverData(canvas, mode, points) {
  ...
  interaction.tooltip.dataset.mode = mode;
  hideAssetChartTooltip(canvas);
}
...
function showAssetChartTooltip(canvas, point) {
  const interaction = assetChartInteractions.get(canvas);
  if (!interaction) return;
